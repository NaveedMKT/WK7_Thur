...
top10 <- data_march2024 %>% 
  ...
  gt() %>% 
  cols_label(
       bnf_item_description = "Prescription Name",
       sum = "Total Prescribed"
     )
top10
```
...
hb <- read_csv("https://www.opendata.nhs.scot/dataset/9f942fdb-e59e-44f5-b534-d6e17229cc7b/resource/652ff726-e676-4a20-abda-435b98dd7bdc/download/hb14_hb19.csv") %>% 
  clean_names()
joined_data <- full_join(hb, data_march2024, by = join_by(hb == hbt))
...
```{r}
joined_data <- full_join(joined_data, population_data)
```
...
```{r}
paracet_per_type <- joined_data %>% 
  group_by(hb_name, bnf_item_description) %>% 
  filter(str_detect(bnf_item_description, "PARACET")) %>% 
  filter(str_detect(bnf_item_description, "CAPLETS|CAPSULES")) %>% 
  summarise(paid_quantity = sum(paid_quantity))

para_per_person <- full_join(population_data, paracet_per_type) %>%
  group_by(hb_name, bnf_item_description) %>% 
  summarise(per_person = paid_quantity/hb_population)

paracet_per_person_bar <- para_per_person %>% 
  ggplot(aes(y = per_person, x = reorder(hb_name, per_person))) +
  geom_col() +
  coord_flip()+
  labs(title = "No. of paracetamol capsules/caplets per health board", x = "Health Board Name", y = "Paracetamol Prescriptions per person")

paracet_per_person_bar
```
...
library(patchwork)
?patchwork
unique(data_apr2020$bnf_item_description)
lockdown_data <- read_csv("https://www.opendata.nhs.scot/dataset/84393984-14e9-4b0d-a797-b288db64d088/resource/9cdc0526-21ab-43de-b832-bc032cd31b24/download/pitc202004.csv") %>% 
  clean_names()

lockdown_data <- full_join(hb, lockdown_data, by = join_by(hb == hbt))

data_apr2020 <- full_join(population_data, lockdown_data)

apr_para <- data_apr2020%>% 
  group_by(hb_name, bnf_item_description) %>% 
  filter(str_detect(bnf_item_description, "PARACET")) %>% 
  filter(str_detect(bnf_item_description, "CAP|TAB")) %>% 
  summarise(paid_quantity = sum(paid_quantity))

apr_per_person <- full_join(population_data, apr_para) %>%
  group_by(hb_name, bnf_item_description) %>% 
  summarise(per_person = paid_quantity/hb_population)

apr_per_person_bar <- apr_per_person %>% 
  ggplot(aes(y = per_person, x = reorder(hb_name, per_person))) +
  geom_col() +
  coord_flip()+
  labs(title = "No. of paracetamol capsules/caplets per health board", x = "Health Board Name", y = "Paracetamol Prescriptions per person") 

apr_per_person_bar + paracet_per_person_bar
```
...
six_month_data <- full_join(hb, consecutive_data, by = join_by(hb == hbt))

six_para <- six_month_data %>% 
  group_by(hb_name, bnf_item_description, paid_date_month) %>% 
  filter(str_detect(bnf_item_description, "PARACET")) %>% 
  filter(str_detect(bnf_item_description, "CAP|TAB")) %>% 
  summarise(paid_quantity = sum(paid_quantity))

six_per_person <- full_join(population_data, six_para) %>%
  group_by(hb_name, bnf_item_description, paid_date_month) %>% 
  summarise(per_person = paid_quantity/hb_population)

six_per_person_line <- six_per_person %>% 
  ggplot(aes(y = per_person, x = reorder(hb_name, per_person))) +
  geom_line() +
